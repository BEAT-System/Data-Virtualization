...
% Last Modified by GUIDE v2.5 24-Apr-2020 13:23:16
...
if strcmp(get(serial_connection,'Status'),'open')% Wenn Verbindung hergestellt ist
    ...
    % Startbyte Senden
    if play == true
        fprintf(serial_connection,(get(handles.edit1,'String')) );
    elseif play == false
        fprintf(serial_connection,'00,00');
    end
    % Buttonfarbe ändern
    set(handles.pushbutton3,'BackgroundColor',[0.94 0.94 0.94]);
    pause(0.5);
	...
	while play == true
        ...
        if serial_connection.BytesAvailable > 0 % Wenn Daten verfügbar sind
            ...
            p = plot(ax,time,rpuffer(:,4:5)); hold off;
            ...
            tex = 'Bpm,P_ins,A1,A2,A3,A4,D1';
            %tex = settings{5,2};
            t = text(tx,ty,[tex,': ', num2str(rpuffer(end,[2 3 6 7 8 9 10]))],'FontSize',11,'Interpreter','none',...
